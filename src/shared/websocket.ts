...
// For live location tracking (unchanged)
    ...
    ws.on("message", async (data: string) => {
      try {
        ...
        switch (parsedData.event) {
          ...
          // ========== SEND SINGLE MESSAGE ==========
          case "message": {
            const { clientRequestId, message, images } = parsedData;
            if (!ws.userId || !clientRequestId || !message) return;
          ...
          // ========== FETCH CHAT HISTORY ==========
          case "fetchChats": {
            const { clientRequestId } = parsedData;
            if (!ws.userId || !clientRequestId) return;
            ...
            await prisma.chat.updateMany({
              where: { clientRequestId, receiverId: ws.userId },
              data: { isRead: true },
            });
          ...
          case "unReadMessages": {
            ...
            const unReadMessages = await prisma.chat.findMany({
              where: { receiverId: ws.userId, isRead: false },
            });
          ...
          // ========== MESSAGE LIST PER CLIENT REQUEST ==========
          case "messageList": {
            if (!ws.userId) return;
